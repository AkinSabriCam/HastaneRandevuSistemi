...
export default class HastaLogin extends React.Component {
  ...
  GirisYap=()=>{
    fetch("http://hastaneapi-6b885d43.francecentral.cloudapp.azure.com/api/Security/Logind?tckn="+this.state.tckn+"&password="+this.state.password)
    .then(data=>{
       if(data.ok){
        
         fetch("http://hastaneapi-6b885d43.francecentral.cloudapp.azure.com/api/Doktor/GetByTcknPassword?tckn="+this.state.tckn+"&password="+this.state.password)
           .then(res=>res.json())
           .then(response=>{
               let doktor =response;
               
               this.props.navigation.navigate("DoktorMenu",{doktorID:doktor.doktorID,doktorAdi:doktor.adi,doktorSoyadi:doktor.soyadi})
           })
           .catch(err=>alert(err));
       }
       else{
         alert(this.state.tckn +this.state.password+ data.status);
       }
    })
    .catch(err=>alert(err))
 }
